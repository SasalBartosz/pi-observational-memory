 ...
 */
export function renderSummary(overview: string | undefined, map: string | undefined, observations: Observation[]): string {
	const sorted = sortObservations(observations);
	const overviewText = overview?.trim();
	if (!overviewText && !map && sorted.length === 0) return "";
